			...
			<div>
				...
				<table class="message table table-bordered">
					...
					<tbody>
						<c:forEach items="${list}" var="dto">
							<tr>
								<td><input type="checkbox" class="cbDelete" name="cbDelete"
									value="${dto.seq}"></td>
								<td>${dto.sname}(${dto.sid})</td>
								<td><c:if test="${dto.state == 1 or dto.state == 0}">
										<b><a href="/codestudy/member/view.do?seq=${dto.seq}">${dto.content}</a></b>
									</c:if> <c:if test="${dto.state == 2}">
				...
				<div class="btns">
					<button type="button" class="btn btn-default"
						onclick="location.href='/codestudy/member/send.do';">쪽지
						보내기</button>
					<button type="button" class="btn btn-default" onclick="">쪽지
						삭제하기</button>
				</div>
