		...
		<div class="content col-sm-12 col-md-9">
			<div>
				...
				<c:if test="${not empty search}">
					<div class="message well well-sm">'${search}'(으)로
						${list.size()}건의 게시물을 검색했습니다.</div>
				</c:if>
				...
				<form id="searchForm" method="GET" action="/codestudy/board/list.do">
					<div class="input-group search">
						<input type="text" class="form-control" placeholder=""
							...
							style="cursor: pointer;" onclick="$('#searchForm').submit();"><span
					...
					<tbody>
						...
						<c:forEach items="${list}" var="dto">
							<tr>
								<td>1</td>
								<td><a
									href="/codestudy/board/view.do?seq=${dto.seq}&search=${search}&page=${nowPage}"
									style="margin-left: ${30 * dto.depth}px;"> <c:if
											test="${dto.depth > 0}">
											<span class="glyphicon glyphicon-share-alt"></span>
										</c:if> ${dto.subject}
								</a> <!-- 첨부파일 유무 --> <c:if test="${not empty dto.filename}">
										<span class="glyphicon glyphicon-floppy-disk"></span>
									</c:if> <!-- 댓글 수 --> <c:if test="${dto.ccount > 0}">
										<span class="badge">${dto.ccount}</span>
									</c:if> <!-- 최신글 유무 --> <c:if test="${dto.gap < 1}">
				...
				<div class="btns btn-group">
					<button type="button" class="btn btn-default"
						onclick="location.href='/codestudy/board/list.do';">
					...
					<c:if test="${not empty id}">
						<button type="button" class="btn btn-default"
							onclick="location.href='/codestudy/board/write.do?reply=n';">
