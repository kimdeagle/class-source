...
<body>
	...
	<script>
		...
		$("#btn1").click(function() {
			...
			//Ajax -> (데이터) -> Server -> (데이터) -> Ajax
			
			$.ajax({
				
				type: "GET", //필수 :: 요청 메서드(GET or POST -> 크다 or 작다)
				url: "/codestudy/ajax/ex04data.do", //필수 :: 응답 프로그램 주소(action)
				//data: "" //선택 :: QueryString(key=value&key=value...)
				async: true, //필수(생략하면 true) :: true(비동기) or false(동기) -> false 쓸 일이 거어어어어어의 없다.
